...
def h(p1,p2):
    # heuristic function having two points p1 and p2, it uses manhattan distance - straight line distance (no diagonals involved)
    x1,y1=p1
    x2,y2=p2
    return abs(x1-x2)+abs(y1-y2) #st line distance
...
def algorithm(draw,grid,start,end):
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            #we assume all edges as 1 so if we want to figure out what temporary g-score of the neighbor would be assuming we travelled to the neighbor from the node we are at then we take whatever distance is this current node and add one to it because we are going one more node over which is a neighbor of this node
